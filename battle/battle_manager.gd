...
func gain_trash(amount:int,random:bool,trash_index:int):
	for i in amount:
		var trash_card:BaseCard
		
		if random:
			trash_card = main.get_card(main.card_list.keys()[trash_cards.pick_random()])
		else:
			trash_card = main.get_card(main.card_list.keys()[trash_index])
